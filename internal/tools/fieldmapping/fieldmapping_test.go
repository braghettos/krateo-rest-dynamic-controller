...
func TestNormalizeResponseBody_JQRefDeferred(t *testing.T) {
	// Module-referenced jq is not yet supported: the entry is left unapplied (not an error).
	body := map[string]interface{}{"x": "raw"}
	verbs := getVerb(getter.FieldMappingItem{
		InResponse:       "x",
		InCustomResource: "status.y",
		ValueMapping:     &getter.ValueMapping{Type: "jq", JQ: &getter.JQProgram{Ref: "configmap://ns/cm/mod.jq", Entrypoint: "f"}},
	})

	require.NoError(t, NormalizeResponseBody(context.Background(), verbs, []string{"get"}, body))
	assert.Equal(t, "raw", body["x"], "module-ref jq is deferred: entry left unapplied")
	_, found, _ := unstructured.NestedFieldNoCopy(body, "y")
	assert.False(t, found, "deferred jq entry must not relocate")
}
...
func TestNormalizeResponseBody_NoOpCases(t *testing.T) {
	...
	require.NoError(t, NormalizeResponseBody(context.Background(), getVerb(), []string{"get"}, body))
	...
	require.NoError(t, NormalizeResponseBody(context.Background(), verbs2, []string{"create"}, body3))
